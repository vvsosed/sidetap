...
If `uv sync` fails on `webrtcvad-wheels`, you are missing a compiler toolchain — but the wheels package exists precisely to avoid that, so a failure means the platform has no wheel. It is imported lazily everywhere, so the suite will still pass without it.
