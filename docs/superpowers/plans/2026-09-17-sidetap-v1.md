...
`ports.py` imports `graph.PwGraph`, which Task 5 creates. Write `graph.py` first if you are doing these out of order; the test in this task also imports `segment.FinalsOnlySegmenter` from Task 15. **Run Task 5 and Task 15 before this task's tests pass.** Write the files now; the `test_ports.py` run is verified at the end of Task 15.
...
Tests for this task are verified at the end of Task 15, once `graph.py` and `segment.py` exist.
...
- [ ] **Step 5: Verify the ports test from Task 4 now passes**

Run: `uv run pytest tests/test_ports.py -v`
Expected: PASS, 2 passed. Every fake now satisfies its Protocol.
