...
class WpctlVolumeControl:
    def set_volume(self, serial: int, fraction: float) -> bool:
        ...
        try:
            result = subprocess.run(
                [require_tool(WPCTL), "set-volume", str(serial), f"{fraction:.2f}"],
                capture_output=True,
                text=True,
                timeout=LINK_TIMEOUT_S,
            )
        ...
        if result.returncode != 0:
            log.debug("wpctl set-volume %s %.2f failed: %s", serial, fraction, result.stderr)
            return False
        return True
...
def test_the_duck_closes_on_speech_and_opens_on_silence():
    ...
    duck = DuckControl(volume, serial=42)
...
def test_the_duck_starts_open():
    ...
    """
    volume = FakeVolumeControl()
    playout = Playout(Direction.IN, FakeAudioSink(), duck=DuckControl(volume, serial=42))
    for _ in range(3):
        playout.tick()
    assert volume.calls == []
...
class DuckControl:
    ...
    def __init__(self, volume: VolumeControl, serial: int):
        self._volume = volume
        self._serial = serial
        self._closed = False
    ...
    def close(self) -> None:
        if not self._closed:
            self._volume.set_volume(self._serial, 0.0)
            self._closed = True

    def open(self) -> None:
        if self._closed:
            self._volume.set_volume(self._serial, 1.0)
            self._closed = False
...
class Session:
    ...
    def setup(self) -> None:
        ...
        for direction, config in configs.items():
            duck = (
                DuckControl(self._volume(), self.router.duck_serial)
                if direction is Direction.IN and self.router.duck_serial is not None
                else None
            )
