...
13. **The two virtual-mic nodes carry different descriptions.** Both showing
...
def test_one_direction_dying_does_not_drop_the_call(tmp_path, zoom_graph):
...
def test_the_session_stops_once_every_direction_is_dead(tmp_path, zoom_graph):
    from google.api_core import exceptions as gexc
...
class Session:
    ...
    def _poll_capture_health(self, stop: threading.Event) -> None:
        ...
        while not stop.is_set():
            for direction in Direction:
                queue = self.capture.queues.get(direction.track)
                if queue is not None:
                    self.metrics.set_capture_dropped(direction, queue.dropped)
            stop.wait(1.0)
