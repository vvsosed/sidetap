...
class SilenceWatch:
    """Fires when a direction has received no captured audio at all.

    Distinct from DeadAirWatch, which watches the OUT direction's *output*.
    This watches either direction's *input*, and covers the failures that
    leave a direction deaf: pw-link missing or broken so the tap never
    connects, the application exiting, or its node going away mid-call.

    The audio fail-safe still holds in that case - the duck only closes while
    speech is being written, so the user hears the remote party untranslated
    rather than silence. But nothing would tell them WHY, and "I can hear them
    but there is no translation" is exactly the report that is impossible to
    debug after the fact. This is what turns the pane red.
    """

    def __init__(self, clock: Clock, threshold_s: float = NO_AUDIO_S):
        self._clock = clock
        self._threshold_s = threshold_s
        self._last_audio: float | None = None

    def heard_audio(self) -> None:
        self._last_audio = self._clock.monotonic()

    def start(self) -> None:
        """Begin the clock without claiming audio was heard."""
    ...
    def alarming(self) -> bool:
        if self._last_audio is None:
            return False
        return (self._clock.monotonic() - self._last_audio) > self._threshold_s
...
class Session:
    ...
    def _poll_capture_health(self, stop: threading.Event) -> None:
        """Copy the capture queues' drop counters into Metrics.

        DroppingQueue only logs. meetscribe's known bug was exactly that: a
        network outage overflowed this queue, the drops were logged, and the
        output carried no marker - so a lost stretch read as nobody talking.
        The lag cap's on_dropped callback covers the PLAYOUT queue, which is a
        ...
        while not stop.is_set():
            for direction in Direction:
                ...
                if track_queue is not None:
                    self.metrics.set_capture_dropped(
                        direction, track_queue.dropped
                    )
            stop.wait(1.0)
    ...
    def refresh_from_metrics(self) -> None:
        ...
        for direction, state in snapshot.directions.items():
            ...
            self.query_one(f"#stats-{suffix}", Static).update(
                ...
                f"{format_latency(state.latency)}"
            )
