...
def check_virtmic(graph: PwGraph, config_path: Path = VIRTMIC_CONFIG_PATH) -> Check:
    ...
    broken and stops reading its output. Observed the first time anyone ran it.

    Tests must pass config_path explicitly: the default is a real path in the
    developer's home directory, so on a machine where sidetap is installed the
    default would silently take the other branch.
    """
