...
| ordinary conversation, speech about half the time | **~$0.96** |
| both parties talking continuously | **~$1.92** |

Translation and synthesis add to this in proportion to words actually spoken,
not to call length. Say plainly that the idle figure is 20x lower than the
talking figure **because of the silence gate**, and that this is why
`doctor` fails loudly when `webrtcvad` is missing rather than degrading
